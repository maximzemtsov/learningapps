...
def converter_():
    ...
    currentImageSourcePath = currentPath + "/img_source/"
    ...
    outPath = currentPath + "/img_result/"
    ...
    deltaPixels = 544.0 / 884  # Соотношение высоты и ширины нужное.
    targetWidth = 884
    targetHeight = 544

    for name, fullPath in imageList.items():
        convert_image_width_add(name, fullPath, outPath, deltaPixels, targetWidth, targetHeight)
...
def convert_image_crop(name, fullPath, outPath, deltaPixels, targetWidth, targetHeight):
    ...
    if im.height / im.width != deltaPixels:
        ...
        # будем увеличивать высоту
        # Найдем сначало размеры холста
        canvas_width = im.width
        canvas_height = im.width * 2
        old_width, old_height = im.size
        # зададим центр
        ...
        newImage.paste(im, (x1, y1, x1 + old_width, y1 + old_height))
        im = newImage
    im.save(outPath + name,dpi=(300, 300))
    pprint('Файл ' + name + " обработан успешно")
